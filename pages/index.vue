<template>
  <div
    ref="container"
    class="container relative bg-gray mx-auto text-black"
    ...
    <div class="px-2">
      <div class="flex items-start justify-start">
        <div
          class="logo w-70"
          v-html="require(`~/assets/icons/logo3.svg?include`)"
          title="VCardFile - A Digital Business Card Generator"
        ></div>
      </div>
        <!-- <a
          class="
    ...
    <div class="md:grid md:grid-cols-2">
      <div class="px-4 mt-32">
        <div ref="create" id="step-1" class="pt-8">
          ...
          <div class="stepC">
            ...
            <Attachment
              ...
              :showAlert="showAlert"
            />
            <p class="mt-6 border p-4 rounded bg-gray text-gray-400">
        ...
        <div id="step-2" class="mt-16">
          ...
          class="mt-6 border p-4 rounded bg-gray text-gray-400">
          ...
          <p
           ...
           class="mt-6 border p-4 rounded bg-gray text-gray-400">
            Recommended profile photo size is 160 x 160 pixels, with an aspect
            ratio of 1:1
          </p>



          <h2 class="font-extrabold text-2xl  mt-6">Logo below Profile</h2>          
          <Attachment
            :content="images"
            type="logo1"
            :resizeImage="resizeImage"
            label="Add logo below profile photo"
            description="suggested format: jpeg, png or gif"
            :showAlert="showAlert"
          />
          <p class="mt-6 border p-4 rounded bg-gray text-gray-400">
            Recommended logo photo size is 100 x 100 pixels, with an aspect
            ratio of 1:1
          </p>
        ...
        <div id="step-4" class="mt-16">
          ...
          <!-- class="stepC actions mt-6 bg-gray"
            :class="{ 'border-t pt-6': secondaryActions.length }" -->
        </div>
          ...
          <div class="stepC mt-3 flex flex-wrap">
            <button
              ...
              :class="
                theme == 1
                  ? 'bg-emerald-600'
                  : 'bg-red-100	 hover:bg-gray-200	 focus:bg-gray-200'
              "
            >
              A
            </button>
        ...
        <div id="step-11" class="mt-16">
         ...
         <h2 class= "mb-6 mt-16 font-extrabold text-2xl"> QR CODE </h2>
        <div class="something border-2" style="padding: 1.3rem; width: 250px;" >
<canvas width="200px" height="200px" id="canvas" style="height: 200px; width: 200px; max-width: 200px; max-height: 200px;"></canvas>
<!-- <button class="inline-block mb-5 bg-emerald-700 leading-none text-xl tracking-wide border-2 border-transparent font-extrabold p-4 rounded mt-6 select-none transition-colors duration-200 hover:bg-emerald-600  focus:bg-emerald-600
 text-white"
@click="downloadQr"
>
Download QRCODE
      ...
      >
        ...
        >
          <div id="device" class="bg-black  rounded sm:mt-10">
            <h2 class="text-center py-4 font-extrabold text-white">
              LIVE PREVIEW
            </h2>
            <div id="browserFrame" class="overflow-hidden flex flex-col">
              ...
              >
                <div id="searchField" class="p-2 flex items-center">
                  <input
                    type="text"
                    class="pl-4 h-12 w-full bg-white rounded text-gray-500"
    ...
    <Vcard ref="vCard" :vCard="vCard" :base64string="generateBase64()" />
    <Footer />
  </div>
</template>
...
export default {
  ...
  if (imgElem && imgElem.src && imgElem.src.indexOf(',') > -1) {
    const canvas = document.createElement('canvas');
    canvas.width = 160;
    canvas.height = 160;
    const ctx = canvas.getContext('2d');
    ctx.drawImage(imgElem, 0, 0, 160, 160);
  ...
  //   },
//     generateQRCode() {
//   let a = this.$refs.vCard.$refs.vCard.innerText.replace(/PHOTO;TYPE=JPEG;ENCODING=BASE64:[^\n]*/, '');
//   let emptyRegex = /^(FN:|ORG:|ADR:|TITLE:|TEL:|EMAIL:|URL:|KEY:|NOTE:)\s*$/m;
  
//   // Check if a contains only empty fields
//   if (emptyRegex.test(a)) {
//     alert("Cannot generate QR code: input is empty.");
//     return;
//   }
  
//   // Inner text is not empty, generate QR code
//   QRCode.toCanvas(document.getElementById('canvas'), a, (error, canvas) => {
...
// },
  ...
  //  let a = this.$refs.vCard.$refs.vCard.innerText.replace(/PHOTO[^:]*:[^;]*;[^,]*,([^\n]*)\n/, '');
  let a = this.$refs.vCard.$refs.vCard.innerText.replace(/PHOTO;TYPE=JPEG;ENCODING=BASE64:[^\n]*/, '');
  // Inner text is not empty, generate QR code
  QRCode.toCanvas(document.getElementById('canvas'), a, (error, canvas) => {
    if (error) console.error(error);
  ...
  type: 'text/plain',
          ...
          // let vCard = new Blob([this.$refs.vCard.$refs.vCard.innerText.replace(/ADR;TYPE=WORK:\s*{{\s*(.*?)\s*}}\s*/g, (match, p1) => "ADR;TYPE=WORK:{{" + p1.replace(/\s+/g, '') + "}}\n")], {
          //   type: 'text/plain',
          // })
          // let guide = new Blob(
          //   [
          ...
          const qrDataURL = document.getElementById('canvas').toDataURL('image/png');
          ...
          zip.folder(username).file(`qrcode.png`, qrDataURL.substr(qrDataURL.indexOf(',') + 1), {base64: true});
